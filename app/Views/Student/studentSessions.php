...
if ($hasSessionTypesToAdd) {
    ...
    foreach ($allSessionTypes as $sessionType) {
        if (!in_array($sessionType['sessionTypeId'], $existingSessionTypeIds)) {
            $content .= "<option value='" . htmlspecialchars($sessionType['sessionTypeId'], ENT_QUOTES, 'UTF-8') . "'>" . htmlspecialchars($sessionType['sessionType'], ENT_QUOTES, 'UTF-8') . "</option>";
        }
    }

    // Aylık Seans Sayısı inputu
    $content .= "
                        </select>
                        <br>
